...
export function setupContainer(): AwilixContainer {
  ...
  // Helper to register from a definitions object (now recursive)
  const registerFromDefs = (defs: any) => {
    Object.keys(defs).forEach(key => {
      ...
      // If it's a nested object (e.g. from a deep auto-load), recurse
      if (typeof item === 'object' && !Array.isArray(item) && !key.endsWith('Repository') && !key.endsWith('Service') && !key.endsWith('UseCase')) {
        registerFromDefs(item);
        return;
      }
